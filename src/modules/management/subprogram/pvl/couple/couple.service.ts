import { BadRequestException, Injectable } from '@nestjs/common';
import { Couple } from '@prisma/client';
import * as xlsx from 'xlsx';
import { CreateCoupleDto, UpdateCoupleDto } from './dto';
import { PrismaService } from '../../../../../prisma/prisma.service';
import {
  paginationHelper,
  timezoneHelper,
} from '../../../../../common/helpers';
import { SearchDto } from '../../../../../common/dto';

@Injectable()
export class CoupleService {
  constructor(private readonly prisma: PrismaService) {}

  async create(dto: CreateCoupleDto): Promise<Couple> {
    const Couple = await this.prisma.couple.create({
      data: {
        ...dto,
        created_at: timezoneHelper(),
        updated_at: timezoneHelper(),
      },
    });
    return await this.getCoupleById(Couple.id);
  }

  async findAll(dto: SearchDto): Promise<any> {
    const { search, ...pagination } = dto;
    const where: any = { deleted_at: null };
    if (search) where.name = { contains: search, mode: 'insensitive' };
    return paginationHelper(
      this.prisma.couple,
      {
        where,
        orderBy: { name: 'asc' },
      },
      pagination,
    );
  }

  async findOne(id: string): Promise<Couple> {
    return await this.getCoupleById(id);
  }

  async update(id: string, dto: UpdateCoupleDto): Promise<Couple> {
    await this.getCoupleById(id);
    await this.prisma.couple.update({
      data: {
        ...dto,
        updated_at: timezoneHelper(),
      },
      where: { id },
    });
    return await this.getCoupleById(id);
  }

  async toggleDelete(id: string): Promise<any> {
    const Couple = await this.getCoupleById(id, true);
    const inactive = Couple.deleted_at;
    const deleted_at = inactive ? null : timezoneHelper();
    await this.prisma.couple.update({
      data: {
        updated_at: timezoneHelper(),
        deleted_at,
      },
      where: { id },
    });
    return {
      action: inactive ? 'Restore' : 'Delete',
      id,
    };
  }

  async upload(file: Express.Multer.File) {
    const count = await this.prisma.couple.count();
    if (count > 0)
      throw new BadRequestException('Solo se puede realizar una vez');
    const workbook = xlsx.read(file.buffer, { type: 'buffer' });
    const sheetName = workbook.SheetNames[0];
    const rows = xlsx.utils.sheet_to_json(workbook.Sheets[sheetName]);
    const data = rows.map((row: any) => {
      return {
        name: row.name,
        created_at: timezoneHelper(),
        updated_at: timezoneHelper(),
      };
    });
    await this.prisma.couple.createMany({ data });
    return { success: true };
  }

  private async getCoupleById(
    id: string,
    toogle: boolean = false,
  ): Promise<any> {
    const Couple = await this.prisma.couple.findUnique({
      where: { id },
    });
    if (!Couple)
      throw new BadRequestException('Centro de acoplo no encontrado');
    if (Couple.deleted_at && !toogle)
      throw new BadRequestException('Centro de acoplo eliminado');
    return Couple;
  }
}
